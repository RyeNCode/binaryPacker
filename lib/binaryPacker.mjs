...
class BinaryPacker {
  ...
  #readDefintions(definition) {
    ...
    for (let defProp in definition) {
      ...
      if (defObj.type === 'complex') {
        ...
        this.#definitions[defProp] = defObj;
      } else if (this.#pendingDefinitions.includes(defObj.type)) ; else if (this.#definitions.hasOwnProperty(defObj.type)) ; else {
   ...
   */
    ...
    if (BinaryPacker.#builtInDefs.includes(type)) {
      ...
      case 'uint64':
        ...
        break;
      case 'float32':
        this.#checkBufferLength(float32.BYTES_PER_ELEMENT);
  ...
  #decodeComponent(type, options) {
    if (BinaryPacker.#builtInDefs.includes(type)) {
      ...
      case 'int16':
        return this.#dataView.getInt16(this.#position);
      case 'uint16':
        return this.#dataView.getUint16(this.#position);
      case 'int32':
        return this.#dataView.getInt32(this.#position);
      case 'uint32':
        return this.#dataView.getUint32(this.#position);
      case 'int64':
        return this.#dataView.getBigInt64(this.#position);
      case 'uint64':
        return this.#dataView.getBigUint64(this.#position);
      case 'float32':
        return this.#dataView.getFloat32(this.#position);
      case 'float64':
        return this.#dataView.getFloat64(this.#position);
...
export { BinaryPacker, BinaryPacker as default };
